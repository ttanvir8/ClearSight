...
class Fog:
    ...
    def get_loaders(self, parse_patches=True, validation='default'):
        print("=> evaluating using Fog dataloader...")
        
        # Validation Data Setup
        if hasattr(self.config.data, 'val_data_dir'):
            val_path = self.config.data.val_data_dir
        else:
            val_path = os.path.join(self.config.data.data_dir, 'data', 'fog_val')
            
        val_filename = getattr(self.config.data, 'val_filelist', 'fog_val.txt')
        print(f"=> Validation path: {val_path}")
        print(f"=> Validation filelist: {val_filename}")

        # Training Data Setup
        train_path = getattr(self.config.data, 'train_data_dir', os.path.join(self.config.data.data_dir, 'data', 'fog_train'))
        train_filelist = getattr(self.config.data, 'filelist', 'fog.txt')
        print(f"=> Training path: {train_path}")
        print(f"=> Training filelist: {train_filelist}")

        # Create Datasets
        train_dataset = FogDataset(train_path,
        ...
        val_dataset = FogDataset(val_path, 
                                 n=self.config.training.patch_n,
                                 patch_size=self.config.data.image_size,
                                 transforms=self.transforms,
...
class FogDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        with open(train_list) as f:
            ...
            # Replace 'input' with 'gt' to get ground truth filenames
    ...
    def get_images(self, index):
        ...
        if self.parse_patches:
            ...
            # Using Resampling.LANCZOS or BICUBIC instead of deprecated ANTIALIAS
