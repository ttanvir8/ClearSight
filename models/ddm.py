        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.step % self.config.training.snapshot_freq == 0 or self.step == 1:
                    ...
                    base_name = os.path.join(self.config.data.data_dir, 'ckpts', self.config.data.dataset + '_ddpm')
                    
                    # Save latest
                    utils.logging.save_checkpoint(state, filename=base_name)
                    
                    # Save snapshot with step
                    if self.step % self.config.training.snapshot_freq == 0:
                        utils.logging.save_checkpoint(state, filename=base_name + f"_{self.step}")
